...
export class Store {
  constructor (options = {}) {
    ...
    // script 标签使用vue 和vuex 时执行安装,模块化应用时在调用方Vue.use(Vuex) 即可
    ...
    // apply plugins（其实插件开发很简单，约定好借口暴露给外边就可以了，vue 插件，vuex 插件，webapck 插件都是如此，插件拿到主题后可注入绑定到钩子上进行其他活动）
...
// 插件安装方法，如果采用script 标签，需要自己调用install
